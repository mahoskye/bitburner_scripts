/**
 * Home Hacking Worker
 * Special worker for home server with target restoration
 *
 * DESIGN: Same as worker.js but includes logic to restore saved target
 * - Checks for /data/last-target.txt on startup
 * - Restores target to port if found
 * - Otherwise behaves identically to worker.js
 *
 * RAM Cost: ~1.85GB (slightly higher due to file operations)
 * Deployment: Only runs on home server
 */
...
export async function main(ns) {
    ...
    ns.tprint(`Home worker started on ${currentServer}`);
